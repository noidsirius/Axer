...
POINT_ACTION_LIMIT="--point-action-limit 11"
OVERSIGHT="--oversight"
MODE=${3:-""}
APP_NAME=${SNAPSHOT%%.S_*}
echo "Snapshot $SNAPSHOT in App $APP_NAME"
# gtimeout $SNAPSHOT_TIMEOUT 
python main.py --app-name "$APP_NAME" --output-path "$RESULT_PATH" --snapshot "$SNAPSHOT" --debug $DIRECTIONAL_ACTION_LIMIT $POINT_ACTION_LIMIT $OVERSIGHT $MODE
